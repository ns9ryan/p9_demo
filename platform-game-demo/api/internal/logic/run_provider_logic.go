...
type RunProviderLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewRunProviderLogic(ctx context.Context, svcCtx *svc.ServiceContext) *RunProviderLogic {
	return &RunProviderLogic{
...
func (l *RunProviderLogic) RunProvider(req *types.SyncRunReq) (resp *types.SyncRunResp, err error) {
	l.Infof("[API RunProvider] received sync run request")

	if l.svcCtx == nil || l.svcCtx.GrpcClient == nil {
		l.Error("[API RunProvider] gRPC client not available")
	...
	if err != nil {
		l.Errorf("[API RunProvider] SyncPreview call failed: %v", err)
		return nil, err
	}

	if previewResp == nil {
		l.Error("[API RunProvider] SyncPreview response is nil")
		return nil, fmt.Errorf("SyncPreview response is nil")
	}

	if previewResp.Code != constant.CodeSuccess {
		l.Errorf("[API RunProvider] SyncPreview error: Code=%d, Message=%s", previewResp.Code, previewResp.Message)
	...
	if err != nil {
		l.Errorf("[API RunProvider] SyncRun call failed: %v", err)
		return nil, err
	}

	if runResp == nil {
		l.Error("[API RunProvider] SyncRun response is nil")
		return nil, fmt.Errorf("SyncRun response is nil")
	}

	if runResp.Code != constant.CodeSuccess {
		l.Errorf("[API RunProvider] SyncRun error: Code=%d, Message=%s", runResp.Code, runResp.Message)
	...
	l.Infof("[API RunProvider] success: preview_total=%d, preview_diffs=%d, created=%d, updated=%d, deleted=%d, failed=%d, skipped=%d",
