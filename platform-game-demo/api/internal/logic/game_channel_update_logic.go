...
func (l *GameChannelUpdateLogic) GameChannelUpdate(req *types.GameChannelUpdateReq) (resp *types.GameChannelResp, err error) {
	...
	grpcResp, err := l.svcCtx.GrpcClient.GetPlatformGameServiceClient().UpdateGameChannel(l.ctx, grpcReq)
	if err != nil {
	...
	resp = &types.GameChannelResp{
		ID:          req.ID,
		ChannelCode: grpcResp.Data.ChannelCode,
		NameI18n:    grpcResp.Data.SourceNameI18N,
