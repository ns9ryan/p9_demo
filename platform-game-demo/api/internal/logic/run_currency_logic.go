...
func (l *RunCurrencyLogic) RunCurrency(req *types.SyncRunReq) (resp *types.SyncRunResp, err error) {
	...
	if err != nil {
		l.Errorf("[API RunCurrency] SyncPreview call failed: %v", err)
		return nil, err
	}

	if previewResp == nil {
		l.Error("[API RunCurrency] SyncPreview response is nil")
		return nil, fmt.Errorf("SyncPreview response is nil")
	}

	if previewResp.Code != constant.CodeSuccess {
		l.Errorf("[API RunCurrency] SyncPreview error: Code=%d, Message=%s", previewResp.Code, previewResp.Message)
	...
	if runResp.Code != constant.CodeSuccess {
		l.Errorf("[API RunCurrency] SyncRun error: Code=%d, Message=%s", runResp.Code, runResp.Message)
	...
	l.Infof("[API RunCurrency] success: preview_total=%d, preview_diffs=%d, created=%d, updated=%d, deleted=%d, failed=%d, skipped=%d",
