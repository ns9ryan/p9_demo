...
func (l *RunGameLogic) RunGame(req *types.SyncRunReq) (resp *types.SyncRunResp, err error) {
	...
	// 检查频率限制（5秒）
	if !l.svcCtx.SyncRateLimiter.CheckRateLimit("game") {
		l.Infof("[API RunGame] sync request rejected due to rate limit (5s)")
	...
	l.Infof("[API RunGame] sync run success: checkpoint_id=%d", runResp.CheckpointId)
