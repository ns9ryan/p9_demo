...
func (l *RunCategoryLogic) RunCategory(req *types.SyncRunReq) (resp *types.SyncRunResp, err error) {
	...
	// 检查频率限制（5秒）
	if !l.svcCtx.SyncRateLimiter.CheckRateLimit("category") {
		l.Infof("[API RunCategory] sync request rejected due to rate limit (5s)")
	...
	l.Infof("[API RunCategory] sync run success: checkpoint_id=%d", runResp.CheckpointId)
