...
func (l *RunChannelLogic) RunChannel(req *types.SyncRunReq) (resp *types.SyncRunResp, err error) {
	...
	if err != nil {
		l.Errorf("[API RunChannel] SyncPreview call failed: %v", err)
		return nil, err
	}

	if previewResp == nil {
		l.Error("[API RunChannel] SyncPreview response is nil")
		return nil, fmt.Errorf("SyncPreview response is nil")
	}

	if previewResp.Code != constant.CodeSuccess {
		l.Errorf("[API RunChannel] SyncPreview error: Code=%d, Message=%s", previewResp.Code, previewResp.Message)
	...
	if runResp.Code != constant.CodeSuccess {
		l.Errorf("[API RunChannel] SyncRun error: Code=%d, Message=%s", runResp.Code, runResp.Message)
	...
	l.Infof("[API RunChannel] success: preview_total=%d, preview_diffs=%d, created=%d, updated=%d, deleted=%d, failed=%d, skipped=%d",
