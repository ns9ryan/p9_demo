...
type RunGameLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewRunGameLogic(ctx context.Context, svcCtx *svc.ServiceContext) *RunGameLogic {
	return &RunGameLogic{
...
func (l *RunGameLogic) RunGame(req *types.SyncRunReq) (resp *types.SyncRunResp, err error) {
	...
	if err != nil {
		l.Errorf("[API RunGame] SyncPreview call failed: %v", err)
		return nil, err
	}

	if previewResp == nil {
		l.Error("[API RunGame] SyncPreview response is nil")
		return nil, fmt.Errorf("SyncPreview response is nil")
	}

	if previewResp.Code != constant.CodeSuccess {
		l.Errorf("[API RunGame] SyncPreview error: Code=%d, Message=%s", previewResp.Code, previewResp.Message)
	...
	if err != nil {
		l.Errorf("[API RunGame] SyncRun call failed: %v", err)
		return nil, err
	}

	if runResp == nil {
		l.Error("[API RunGame] SyncRun response is nil")
		return nil, fmt.Errorf("SyncRun response is nil")
	}

	if runResp.Code != constant.CodeSuccess {
		l.Errorf("[API RunGame] SyncRun error: Code=%d, Message=%s", runResp.Code, runResp.Message)
	...
	l.Infof("[API RunGame] success: preview_total=%d, preview_diffs=%d, created=%d, updated=%d, deleted=%d, failed=%d, skipped=%d",
