...
type PreviewGameLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewPreviewGameLogic(ctx context.Context, svcCtx *svc.ServiceContext) *PreviewGameLogic {
	return &PreviewGameLogic{
...
func (l *PreviewGameLogic) PreviewGame(req *types.SyncPreviewReq) (resp *types.SyncPreviewResp, err error) {
	...
	if grpcResp == nil {
		l.Error("[API PreviewGame] gRPC response is nil")
		return nil, fmt.Errorf("gRPC response is nil")
	}

	if grpcResp.Code != constant.CodeSuccess {
		l.Errorf("[API PreviewGame] gRPC error: Code=%d, Message=%s", grpcResp.Code, grpcResp.Message)
	...
	l.Infof("[API PreviewGame] success: remoteTotal=%d, localTotal=%d, createTotal=%d, updateTotal=%d, page=%d, pageSize=%d, filtered=%d, totalPage=%d",
		grpcResp.Stats.RemoteTotal, grpcResp.Stats.LocalTotal, grpcResp.Stats.CreateTotal, grpcResp.Stats.UpdateTotal, page, pageSize, grpcResp.Stats.DiffTotal, resp.TotalPage)
	return resp, nil
}
