...
// Config 国际化配置
type Config struct {
	DefaultLanguage string `json:",default=zh-CN"` // 默认语言
}
...
func (t *Translator) Translate(languageCode, key string, data map[string]any) string {
	// 未指定语言时使用默认语言
	if languageCode == "" {
...
func (t *Translator) T(ctx context.Context, key string) string {
	return t.Translate(Lang(ctx, &t.defaultLanguage), key, nil)
}

// Tf 根据上下文中的语言翻译消息
func (t *Translator) Tf(ctx context.Context, key string, data map[string]any) string {
	return t.Translate(Lang(ctx, &t.defaultLanguage), key, data)
}
