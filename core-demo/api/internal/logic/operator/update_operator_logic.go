package operator

import (
	"context"

	"oa.98ent.com/p9/core/api/internal/convert"
	"oa.98ent.com/p9/core/api/internal/svc"
	"oa.98ent.com/p9/core/api/internal/types"

	"github.com/zeromicro/go-zero/core/logx"
)

type UpdateOperatorLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewUpdateOperatorLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateOperatorLogic {
	return &UpdateOperatorLogic{
		Logger: logx.WithContext(ctx),
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

func (l *UpdateOperatorLogic) UpdateOperator(req *types.UpdateOperatorReq) (resp *types.BaseMsgResp, err error) {
	_, err = l.svcCtx.Core.UpdateOperator(l.ctx, convert.UpdateOperatorReq(req))
	if err != nil {
		return nil, err
	}
	return &types.BaseMsgResp{Result: "success"}, nil
}
