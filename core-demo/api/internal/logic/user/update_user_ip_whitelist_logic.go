package user

import (
	"context"

	"oa.98ent.com/p9/core/api/internal/convert"
	"oa.98ent.com/p9/core/api/internal/svc"
	"oa.98ent.com/p9/core/api/internal/types"

	"github.com/zeromicro/go-zero/core/logx"
)

type UpdateUserIpWhitelistLogic struct {
	logx.Logger
	ctx    context.Context
	svcCtx *svc.ServiceContext
}

func NewUpdateUserIpWhitelistLogic(ctx context.Context, svcCtx *svc.ServiceContext) *UpdateUserIpWhitelistLogic {
	return &UpdateUserIpWhitelistLogic{
		Logger: logx.WithContext(ctx),
		ctx:    ctx,
		svcCtx: svcCtx,
	}
}

func (l *UpdateUserIpWhitelistLogic) UpdateUserIpWhitelist(req *types.UpdateUserIpWhitelistReq) (resp *types.BaseMsgResp, err error) {
	_, err = l.svcCtx.Core.UpdateUserIpWhitelist(l.ctx, convert.UpdateUserIpWhitelistReq(req))
	if err != nil {
		return nil, err
	}
	return &types.BaseMsgResp{Result: "success"}, nil
}
