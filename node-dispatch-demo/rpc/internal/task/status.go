...
func (s *Service) MarkRunning(ctx context.Context, taskNo string, runNo int64, nodeCode string) error {
	// 查询调度任务
	taskData, err := s.db.DispatchTask.Query().Where(dispatchtask.TaskNoEQ(taskNo)).Only(ctx)
	if err != nil {
	...
	// 已经开始或结束的执行记录不重复处理
	if runData.Status != StatusPending {
		return nil
	}
	...
	affected, err := tx.DispatchTaskRun.
		...
		Where(
			...
			dispatchtaskrun.StatusEQ(StatusPending),
		).
	...
	_, err = tx.DispatchTask.
		...
		Where(
			...
			dispatchtask.StatusEQ(StatusPending),
		).
		SetStatus(StatusRunning).
		Save(ctx)
	if err != nil {
